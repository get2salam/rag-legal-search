...
def ingest(
    ...
    chunker = get_chunker(
        chunk_strategy, chunk_size=chunk_size, overlap=min(200, chunk_size // 5)
    )
    chunked_cases = []
    for case in cases:
        text = case.get("text", "")
    ...
    for i in range(0, len(chunked_cases), batch_size):
        ...
        vector_store.add_documents(documents=batch, embeddings=vectors, ids=ids)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Ingest documents into RAG Legal Search"
    )
    parser.add_argument(
        "--source", required=True, help="Path to source file or directory"
    )
    parser.add_argument(
        "--format",
        default="json",
    ...
    parser.add_argument(
        "--chunk-size", type=int, default=1000, help="Chunk size in chars"
    )
    parser.add_argument(
        "--chunk-strategy",
        default="recursive",
