...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        with col1:
            year_from = st.number_input("From Year", min_value=1900, max_value=2026, value=2000)
        with col2:
            year_to = st.number_input("To Year", min_value=1900, max_value=2026, value=2026)
...
def main():
    ...
    # Main content
    st.markdown('<p class="main-header">🔍 RAG Legal Search</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">AI-powered semantic search for legal case law</p>', unsafe_allow_html=True)
